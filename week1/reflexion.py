...
You are a senior engineer applying reflexion to fix the function based on failing tests.
Given the prior implementation, failing test cases, and the required rules inferred from failures,
produce ONLY a single fenced Python code block with an improved implementation of
is_valid_password(password: str) -> bool that passes all tests.
"""
...
def completed_build_reflexion_context(prev_code: str, failures: List[str]) -> str:
    return (
        "Previous implementation of is_valid_password:\n\n"
        + f"""```python
{prev_code}
```"""
        + "\n\nFailing tests and diagnostics:\n- "
        + "\n- ".join(failures)
        + "\n\nUpdate the function to fix all failures. Output only a fenced Python code block."
    )
...
if __name__ == "__main__":
    # Students should start with SYSTEM_PROMPT, YOUR_REFLEXION_PROMPT, and your_build_reflexion_context.
